...
from documenter_pull_request_agent.template import get_documenter_pull_request_agent_template
...
@dataclass
class DocumenterPullRequestAgentDependencies:
    config: YAMLConfig
...
async def run_command(ctx: RunContext[DocumenterPullRequestAgentDependencies], command: list[str]) -> str:
    """
...
def create_documenter_pull_request_agent(pydantic_ai_model: OpenAIModel) -> Agent:
    """
    ...
    base_system_prompt = get_documenter_pull_request_agent_template()

    agent = Agent(
        model=pydantic_ai_model,
        system_prompt=base_system_prompt,
        deps_type=DocumenterPullRequestAgentDependencies,
        instrument=True,
    ...
    print(
        f"DocumenterAI pull request Agent created with model: {pydantic_ai_model.model_name}")
    return agent


# Export necessary components
__all__ = ["create_documenter_pull_request_agent", "DocumenterPullRequestAgentDependencies"]
