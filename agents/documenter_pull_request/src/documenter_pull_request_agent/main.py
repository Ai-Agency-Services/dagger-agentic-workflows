...
from documenter_pull_request_agent.core.documenter_pull_request_agent import (
    DocumenterPullRequestAgentDependencies, create_documenter_pull_request_agent)
from documenter_pull_request_agent.utils import create_llm_model, get_llm_credentials
from pydantic_ai import Agent, UnexpectedModelBehavior
from typing_extensions import Doc


@object_type
class DocumenterPullRequestAgent:
    config: dict
    container: dagger.Container

    @classmethod
    async def create(cls, config_file: Annotated[dagger.File, Doc("Path to YAML config file")]) -> 'DocumenterPullRequestAgent':
    ...
    async def run(
        ...
        async def _run_agent(
            ...
            try:
                self._setup_logging()
                deps = DocumenterPullRequestAgentDependencies(
                    config=self.config,
                    container=container,
                ...
                agent: Agent = create_documenter_pull_request_agent(
                ...
                self.logger.error(
                    f"Error creating DocumenterPullRequestAgentDependencies: {agent_error}")
                print(f"Error running Documenter Pull Request Agent: {agent_error}")
        ...
        try:
            ...
            self.logger.error(f"Error creating DocumenterPullRequestAgent: {e}")
