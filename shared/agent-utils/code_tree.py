...
try:
    from .project_files import FileTreeNode  # package-style
except Exception:
    from project_files import FileTreeNode  # module-style for direct import
...
# Keep in sync with TS unimportantExtensions (adapted)
        ...
        if node.type == 'directory':
            ...
            for ext in UNIMPORTANT_EXTS_OR_DIRS:
                if ext.startswith('/') and ext.endswith('/') and ext.strip('/') in dir_path:
                    return False
            # keep if any important children
...
def _prune_file_token_scores(
    ...
    idx = 0
    # Remove in batches to reduce recompute overhead
...
def truncate_file_tree_based_on_token_budget(
    ...
    # Pass 1: remove unimportant files/dirs
