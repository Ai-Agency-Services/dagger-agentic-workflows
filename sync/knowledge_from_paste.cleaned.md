...
- Comprehensive test fixtures centralized (via shared pytest plugin or tests/conftest.py)
...
# Run all unit tests (repo root)
make test-unit

# Run integration tests
make test-integration

# Run with coverage (repo root)
make test-coverage
...
Important: constructor args (e.g., --config-file, --neo-data) come before the function; method args come after. Use `--mod <module-dir>` so Dagger finds the module's dagger.json. Use `export --path` to write files to the host.
...
# Analyze codebase (build the graph from a repository URL)
# (constructor-first + --mod)
...
# (Note) Removed erroneous web_scraped_content block that contained a 404 scrape
