...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Index code files for RAG (for local testing)")
    parser.add_argument("--repo-path", required=True,
                        help="Path to the local repository to mount into Dagger")
    parser.add_argument("--supabase-url", required=True, help="Supabase URL")
    parser.add_argument("--supabase-key-env", required=True,
                        help="Environment variable name for Supabase API key")
    parser.add_argument("--openai-key-env", required=True,
    ...
    parser.add_argument(
        "--config-path", help="Path to YAML config file (optional)")

    args = parser.parse_args()

    async def main_local():
        # This is a simplified local runner for testing the indexing logic.
        # It requires Dagger to be installed and running.
        try:
            print("Starting local Dagger client for testing...")
            # Quieter Dagger logs
            async with dagger.Connection(dagger.Config(log_output=open(os.devnull, 'w'))) as client:
                ...
                if not os.path.isdir(repo_abs_path):
                    print(
                        f"Error: Repository path {repo_abs_path} not found or not a directory.")
                    return

                print(f"Mounting local repository: {repo_abs_path}")
                src_dir = client.host().directory(repo_abs_path)

                # Basic container, assuming python and find are available
                # You might need a more specific base image if your code_parser has other deps
                container = client.container().from_(
                    "python:3.10-slim").with_mounted_directory("/src", src_dir).with_workdir("/src")

                supabase_key_val = os.getenv(args.supabase_key_env)
                openai_key_val = os.getenv(args.openai_key_env)

                if not supabase_key_val:
                    print(
                        f"Error: Supabase key env var {args.supabase_key_env} not set.")
                    return
                if not openai_key_val:
                    print(
                        f"Error: OpenAI key env var {args.openai_key_env} not set.")
                    return

                supabase_secret = client.set_secret(
                    "supabase-key", supabase_key_val)
                openai_secret = client.set_secret("openai-key", openai_key_val)

                file_exts = args.extensions.split(',')

                # Load YAML config if provided
                cfg = YAMLConfig()  # Default config
                if args.config_path:
                    try:
                        import yaml
                        with open(args.config_path, 'r') as f:
                            ...
                            cfg = YAMLConfig(**user_config_dict)
                        print(f"Loaded configuration from {args.config_path}")
                    except Exception as e:
                        print(
                            f"Warning: Could not load or parse config {args.config_path}: {e}. Using default config.")

                print("Starting codebase indexing...")
                await index_codebase(
                    supabase_url=args.supabase_url,
                    open_ai_key=openai_secret,
                    supabase_key=supabase_secret,
                    file_extensions=file_exts,
                    container=container,
                    config=cfg
                )
                print("Local indexing test finished.")

        except Exception as e:
            print(f"Local Dagger test run failed: {e}")
            traceback.print_exc()
