...
import dagger
from document.template import get_documenter_agent_template
from pydantic_ai import Agent, RunContext
from pydantic_ai.models.openai import OpenAIModel
...
async def scan_agents_directory(ctx: RunContext[DocumenterAgentDependencies], directory: str = "agents") -> str:
    ...
    Args:
        ...
        directory: The directory to scan for agents (default: "agents")
    
    Returns:
        A string containing the found agent files
    """
    work_dir = ctx.deps.config.container.work_dir  # <- access from your YAML config
    find_command = ["bash", "-c", f"find {work_dir}/agents -type f -name '*.py'"]
